...
- [x] HTML의 JavaScript 자산 버전을 `20260903-first-login`으로 갱신
...
현재 비밀번호 변경 대상 사용자는 새 로그인 화면에서 로그인하면 `초기 비밀번호 변경` 폼으로 전환된다. 기존 보안 설정의 `현재 비밀번호` 입력란은 MFA 등록용이며 비밀번호 변경 입력란이 아니다.
