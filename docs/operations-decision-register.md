...
| 이벤트 발송기 | 미결정 | 업무/인프라 관리자 | 메일·알림·전자결재 endpoint, 멱등·재시도 정책 | Outbox publisher, backoff, dead-letter | 실제 이벤트 전달·재시도·중복 방지 통과 |
| 경보 채널 | 미결정 | 운영 관리자 | endpoint, 당직자, 심각도·escalation | alerting manifest, operational health | 장애 경보 수신·확인 증거 |
| 운영 URL·TLS·DNS | 도메인 결정 / 연결 대기 | 인프라 관리자 | `sqcm.safe-link.co.kr`, 가비아 DNS, TLS 인증서, 프록시 | secure cookie·OIDC redirect precheck | Phase 49 승인 후 DNS 연결, 외부 health·핵심 smoke 통과 |
