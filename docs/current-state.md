...
MFA 로그인 간소화(9/14): 사용자 요청에 따라 Production의 최초 TOTP MFA 등록 의무를 제거했다. MFA 미등록 사용자는 회사 이메일과 비밀번호로 바로 로그인할 수 있고, 보안 설정의 선택형 TOTP 등록 및 이미 MFA를 활성화한 계정의 기존 MFA 검증은 유지한다. 비밀번호 해시·CSRF·secure session·RBAC·로그인 rate limit은 변경하지 않았다. 배포 revision `189edd749f8d40cdce03e6cc1938c2d1fe35979d`, GitHub quality `34796291677` SUCCESS, unit 1039건(1031 PASS·8 SKIP·0 FAIL), UI 계약 47 PASS, 격리 Docker 실제 로그인 PASS다. Production Docker 3/3 healthy, database 컨테이너 불변, backend/database host port 0, Inventory와 SQCM-i 보호 Tunnel 연결 각 4 및 공개 응답을 보존했다. 공개 브라우저는 강제 등록 카드가 아닌 이메일·비밀번호 로그인 화면을 표시한다. P7 운영 인수는 별도 증거가 남아 **7/8** 유지. 증거 `P7_OPTIONAL_MFA_LOGIN_SIMPLIFICATION_EVIDENCE.json`, 보고서169.
...
## 2026-09-07 SQCM-i C 제품 고도화 — C1~C4 완료 / C5 G0~G3 완료·G4 입력 Gate PASS·실제 HOLD
...
- C5 G4 `PE-C5-G4-PROVIDER-INPUT-PREFLIGHT` 입력 Gate를 증거 있는 완료로 닫았다. 김무빈 차장의 지시는 5항목 확인·합성시험 준비의 조건부 승인으로 기록하고, 제품명·공급자·API version, staging HTTPS endpoint, 공급자별 필드 매핑, C5 signing Secret reference, UAT 실행·결과서명·rollback 책임자와 승인큐를 기계적으로 검사한다. 집중 16/16, 구문 464개, 전체 단위 962 PASS·8 SKIP, Harness 오류 0이 PASS했다. 현재 실제 입력 5/5와 staging UAT release 승인은 미확정이므로 preflight는 예상대로 `HOLD_EXTERNAL_INPUT`·exit 2다. 구현 체크포인트 `f1c637293efc16c75ebda564ccf79b4a1429f95b`을 원격 동일 branch에 push해 SHA 일치를 확인했다. Secret 값·외부 호출·배포·결재/자금 변경은 0이며 다음 제품 READY는 `PE-C5-G4-ACTUAL-PROVIDER-INPUT-BINDING`이다.
...
- `ACC-P6-83`을 완료했다. 2분 containment reserve를 route-disable 50초, ingress orphan recovery 50초, child 종료 유예 합계 10초, orchestration 여유 10초로 분할했다. 상위 timeout 확장값으로도 이 상한을 늘릴 수 없고 합계를 보장하지 못하는 작은 reserve는 runner 생성 전에 거부한다. failure-first 2건, focused 31 PASS·1 SKIP, 구문 414/414, 저장소 단위 856 PASS·8 SKIP, cutover preflight local blocker 0, GitHub quality `33653761361`이 PASS했다. 실제 containment·DNS/TLS·역할 UAT·P6 cutover·P7 활성화는 `NOT_RUN`이다.
- `ACC-P6-84`를 완료했다. actual P6 조립기의 step·Gate receipt, 세 역할 결과와 업무·보안·운영 서명 시각을 변경창 시작부터 22:00 rollback cutoff까지로 제한했다. 22:00 이후 문서는 형식·SHA가 정상이어도 Production GO 후보로 승격되지 않는다. failure-first 2건, focused 33/33, 구문 414/414, 저장소 단위 858 PASS·8 SKIP, GitHub quality `33655224048`이 PASS했다. 실제 receipt·DNS/TLS·역할 UAT·P6 cutover·P7 활성화는 `NOT_RUN`이다.
- `ACC-P6-85`를 완료했다. runtime receipt JSON에 1~26 sequence를 기록하고 actual P6 조립기가 14개 step·12개 Gate receipt의 연속·고유 sequence와 정해진 identity 순서를 검증한다. 중복·누락·교환은 fail-closed한다. failure-first 2건, focused 51 PASS·1 Windows SKIP, 구문 414/414, 저장소 단위 859 PASS·8 SKIP, GitHub quality `33656557105`가 PASS했다. 실제 receipt·DNS/TLS·역할 UAT·P6 cutover·P7 활성화는 `NOT_RUN`이다.
- `ACC-P6-86`을 완료했다. actual P6 조립기가 runtime receipt의 물리 파일명을 payload의 canonical UTC checkedAt·4자리 sequence·kind·Gate·step과 정확히 대조한다. 파일명 sequence·시각·kind 변조는 fail-closed한다. failure-first 1건, focused 29 PASS·1 Windows SKIP, 구문 414/414, 저장소 단위 860 PASS·8 SKIP, GitHub quality `33657975871`이 PASS했다. 실제 receipt·DNS/TLS·역할 UAT·P6 cutover·P7 활성화는 `NOT_RUN`이다.
- `ACC-P6-87`을 완료했다. 1~26 receipt checkedAt의 비감소 순서, 역할 결과와 role-core-smoke receipt의 exact 시각, rollback Gate 이후부터 signoff-preflight receipt 이전까지의 실제 서명 경계를 검증한다. failure-first 2건, focused 31 PASS·1 Windows SKIP, 구문 414/414, 저장소 단위 862 PASS·8 SKIP, GitHub quality `33659125646`이 PASS했다. 실제 receipt·DNS/TLS·역할 UAT·P6 cutover·P7 활성화는 `NOT_RUN`이다.
...
`ACC-P6-91`에서 세 실제 서명을 사람이 검토한 저장소 밖 물리 unsigned request bundle SHA-256에 추가로 결박했다. actual assembler는 bundle 전체 provenance·세 unsigned payload·signer instruction을 재검증하고 cutover resume는 이 물리 bundle 참조 없이는 Gate 12 전 최종화를 열지 않는다. failure-first 3건, focused 36/36, 구문 417/417, 단위 870 PASS·8 SKIP, GitHub quality `33665080041`이 PASS했으며 실제 request·서명·DNS/TLS·cutover는 `NOT_RUN`이다.

`ACC-P6-92`에서 BUSINESS·SECURITY·OPERATIONS 실제 서명을 별도 물리 MFA 승인 receipt SHA-256에 결박했다. 세 receipt는 동일 run·release·request set·request bundle·signer·signedAt, MFA verified provider identity와 고유 receiptId를 요구하며 누락·미검증·SHA 불일치에서는 Gate 12 전에 대기하거나 actual assembler가 fail-closed한다. focused 46/46, 구문 417/417, 단위 871 PASS·8 SKIP, Harness verify와 GitHub quality `33666946831`의 unit·three-tier가 PASS했으며 실제 MFA receipt·서명·DNS/TLS·cutover는 `NOT_RUN`이다.
