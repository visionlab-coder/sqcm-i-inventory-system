...
| C5 | 회사 시스템 연동 | HR 이동·퇴사와 ERP/전자결재를 중복 입력 없이 연결 | 승인된 API/Webhook, 서명 검증, outbox, 재처리·감사 | 진행 중 · G0~G3 PASS / G4 입력 Gate PASS·실제 HOLD |
...
| G3 | ERP·전자결재 delivery | 기존 outbox를 승인 endpoint에 서명 전송하고 receipt·retry·dead-letter·관리자 재처리를 검증 | 증거 있는 완료 |
| G4 | 공급자 UAT·배포 | 승인 공급자·endpoint·Secret reference로 정상·변조·중복·timeout·rollback 실제 증거 확보 | 입력 Gate PASS · 실제 외부 입력 대기 |

현재 제품 READY는 `PE-C5-G4-ACTUAL-PROVIDER-INPUT-BINDING`이다. G4 입력 Gate는 제품·공급자·API version, staging HTTPS endpoint, 필드 매핑, C5 Secret reference, 시험 책임자와 승인큐를 fail-closed로 검증한다. 집중 16/16, 전체 단위 962 PASS·8 SKIP, 구문 464개와 Harness 오류 0이 PASS했으며 현재 실제 입력은 5/5 MISSING, 승인큐는 조건부이므로 `HOLD_EXTERNAL_INPUT`이다. C5 전체 완료는 아니며 외부 호출·배포·결재/자금 변경은 수행하지 않았다.
