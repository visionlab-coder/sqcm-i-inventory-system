...
| C5 | 회사 시스템 연동 | HR 이동·퇴사와 ERP/전자결재를 중복 입력 없이 연결 | 승인된 API/Webhook, 서명 검증, outbox, 재처리·감사 | 진행 중 · G0~G3 PASS |
...
현재 제품 READY는 `PE-C5-G4-ACTUAL-PROVIDER-UAT-AND-DEPLOYMENT`다. G3는 HTTPS 전용 HMAC publisher, 검증된 receipt·응답 SHA-256, 안전한 실패 코드, 열 번째 dead-letter와 ADMIN 조직 범위 재처리·감사를 구현했다. 전체 단위 958 PASS·8 SKIP, 로컬 application migration 30/30과 합성 signed delivery·receipt·dead-letter·재처리 감사·cleanup 0이 PASS했다. C5 전체 완료는 아니며 실제 연동 전에 ERP·전자결재 공급자, endpoint, 필드 매핑, Secret reference, 시험 담당자 승인이 필요하다.
