...
FR-025는 요청 유형별 확장 필드를 위한 기존 `payload JSONB NOT NULL`을 사용한다. `estimatedAmount`는 소수 둘째 자리 문자열, `quantity`는 정수, `neededAt`은 `YYYY-MM-DD`로 정규화한다. 이번 기능은 스키마를 변경하지 않아 기존 migration checksum을 유지한다.
