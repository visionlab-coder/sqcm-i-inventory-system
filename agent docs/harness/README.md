...
- `operations:activation-orchestrator -- --execute`는 실제 P6 cutover·P7 활성화·Production GO, 저장소 밖 물리 P6 증거·승인 manifest·receipt root와 exact 확인이 모두 있을 때만 19단계 운영 활성화 순서의 다음 한 단계만 실행한다. 승인 manifest는 19개 step과 10개 외부 행위를 정확히 고정하고 최대 45일만 유효하다. receipt에는 원문 대신 stdout/stderr SHA만 기록하며 sequence·command·digest·runId 변조를 거부한다. 동일 run의 single-writer lease를 `wx`로 먼저 획득하므로 중첩 Heartbeat는 `READY_WAIT_OPERATIONS_ACTIVATION_LEASE`로 대기하며, owner만 정상 해제할 수 있고 crash stale lock은 자동 삭제하지 않는다. `WAIT`는 4자리 attempt 영수증으로 9999회까지 같은 단계에서 재개하고 동일 단계 `FAIL` 3회면 `PAUSED`하며, 조건 전에는 child·approval read·lease·receipt write가 모두 0건이다.
