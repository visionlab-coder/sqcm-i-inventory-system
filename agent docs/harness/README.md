...
- `production:cutover-execute -- --resume-signoff --finalize-actual-evidence`는 Gate 12 뒤 동일 run의 26 receipt·세 역할 결과·세 identity 서명을 actual assembler/finalizer에 연결한다. 저장소 밖 원자 기록까지 성공해야 `productionGo=true`이며 조립·검증·쓰기 실패는 exact public route-disable receipt를 요구한다.
