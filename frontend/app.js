...
async function renderCostControl() {
  ...
  const aggregateNotice = roi?.visibility?.organizationAggregatesRestricted
    ? '부서 조회 권한에서는 조직 전체 예산·공급사 집계를 표시하지 않습니다.'
    : !roi ? '비용 집계를 불러오지 못했습니다. 잠시 후 다시 확인하세요.' : '';
  const vendorRows = aggregateNotice ? `<tr><td colspan="5" class="empty-cell">${escapeHtml(aggregateNotice)}</td></tr>` : (roi?.vendors || []).slice(0, 8).map(row => `<tr><td>${escapeHtml(row.name)}</td><td>${row.order_count}</td><td>${money(row.ordered_amount)}</td><td>${Number(row.avg_lead_days || 0).toFixed(1)}일</td><td>${money(row.repair_cost)}</td></tr>`).join('');
  const budgetRows = aggregateNotice ? `<tr><td colspan="4" class="empty-cell">${escapeHtml(aggregateNotice)}</td></tr>` : (roi?.budgets || []).map(row => `<tr><td>${escapeHtml(row.cost_center)}</td><td>${money(row.budget)}</td><td>${money(row.spent)}</td><td>${money(row.remaining)}</td></tr>`).join('');
  $('#view-root').innerHTML = `<div class="page-heading"><div><p class="eyebrow">COST COMMAND CENTER / TCO</p><h1>구매 전 비용 의사결정</h1><p class="muted">취득·수리·이동·처분 원장을 합산해 현재 자본이 묶인 곳과 다음 검토 대상을 보여줍니다.</p></div><button class="secondary" data-go="assets">자산 원장 열기</button></div>
    <p class="muted">수리비는 담당자가 실제 비용으로 입력해 원장에 반영한 금액입니다. 요청의 예상 수리비와 아직 확인하지 않은 과거 비용은 포함하지 않습니다. 결제 완료를 의미하지 않습니다.</p>
    <section class="metric-strip cost-metric-strip"><article class="metric-feature"><span>TCO 원장 합계</span><strong>${money(s.tco)}</strong><small>기록된 비용 이벤트 기준</small></article><article><span>장부가</span><strong>${money(s.book_value)}</strong><small>직선법 감가 반영</small></article><article><span>유휴 자본</span><strong>${money(s.idle_capital)}</strong><small>${s.idle_asset_count || 0}개 자산</small></article><article class="metric-risk"><span>수리 비용</span><strong>${money(s.repair_cost)}</strong><small>실제 입력 비용 원장 누계</small></article></section>
...
async function renderAssetRegister() {
  ...
  $('#view-root').innerHTML = `<div class="page-heading"><div><p class="eyebrow">ASSET ONBOARDING</p><h1>자산 등록 센터</h1><p class="muted">기존 문서와 자산 원장을 안전하게 읽어 표준 자산목록으로 변환하거나, 개별 자산을 등록합니다.</p></div></div>
  <section class="panel import-panel" aria-labelledby="asset-import-title">
    <div class="import-intro"><p class="eyebrow">DOCUMENT INTELLIGENCE / SAFE PREVIEW</p><h2 id="asset-import-title">문서에서 자산목록 자동 등록</h2><p>악성코드 검사와 문서 변환 후 모든 행을 검증합니다. 오류가 한 건이라도 있으면 데이터는 저장되지 않습니다.</p><ol><li>회사 문서 또는 템플릿 선택</li><li>자동 추출 결과와 오류 확인</li><li>사용자가 확정한 경우에만 원장 반영</li></ol></div>
    <form id="asset-import" class="import-form">
      <div class="import-actions"><a class="secondary button-link" href="/api/enterprise/assets/import/template.csv">등록 템플릿 받기</a><label class="file-field"><span>자산 문서 선택</span><input type="file" name="assetDocument" accept=".csv,.txt,.md,.xls,.xlsx,.doc,.docx,.hwp,.hwpx,.ppt,.pptx,.pdf,.jpg,.jpeg,.png,.zip" required aria-describedby="asset-import-help"></label><button class="primary" type="submit">자동 추출·미리보기</button></div>
      <p id="asset-import-help" class="muted">CSV·TXT·MD·Excel·Word·한글·PowerPoint·PDF·JPEG·PNG·ZIP, 최대 10 MiB·500개. 레거시 문서와 이미지/PDF는 연결된 OCR을 사용합니다.</p>
  ...
  importForm.elements.assetDocument.addEventListener('change', () => { previewState = null; resultBox.innerHTML = '<p class="empty-cell">자동 추출·미리보기를 실행해 등록 가능 여부를 확인하세요.</p>'; });
  importForm.addEventListener('submit', async event => {
    event.preventDefault(); const file = importForm.elements.assetDocument.files[0]; const button = event.submitter;
    if (!file) return showMessage('자산 문서를 선택하세요.', 'error');
    if (file.size > 10 * 1024 * 1024) return showMessage('문서 파일은 10 MiB 이하여야 합니다.', 'error');
    button.disabled = true; resultBox.innerHTML = '<p class="loading">악성코드 검사, 문서 추출, 전체 행 검증을 진행하고 있습니다…</p>';
    try {
      const response = await uploadBinary('/api/enterprise/assets/import/document/preview', file); const preview = response.preview;
      previewState = { file, sourceChecksum: response.sourceChecksum, checksum: preview.checksum, invalid: preview.summary.invalid };
      ...
      resultBox.innerHTML = `<div class="import-summary ${preview.summary.invalid ? 'has-errors' : 'ready'}"><strong>${escapeHtml(response.document.originalName)} · ${escapeHtml(response.document.extraction)}</strong><span>전체 ${preview.summary.total}건</span><span>등록 가능 ${preview.summary.valid}건</span><span>수정 필요 ${preview.summary.invalid}건</span></div><div class="table-wrap"><table><thead><tr><th>문서 행</th><th>자산번호</th><th>자산명</th><th>검증 결과</th></tr></thead><tbody>${rows}</tbody></table></div>${preview.rows.length > 100 ? '<p class="muted">화면에는 처음 100건만 표시됩니다. 전체 행은 서버에서 모두 검증했습니다.</p>' : ''}<div class="import-confirm"><p>${preview.summary.invalid ? '원본 문서의 오류 행을 수정한 뒤 다시 미리보기 하세요.' : '모든 행이 유효합니다. 확정하면 하나의 트랜잭션으로 등록됩니다.'}</p><button id="asset-import-commit" class="primary" type="button" ${preview.summary.invalid ? 'disabled' : ''}>${preview.summary.valid}개 자산 등록 확정</button></div>`;
      $('#asset-import-commit')?.addEventListener('click', async clickEvent => {
        ...
        try { const committed = await uploadBinary('/api/enterprise/assets/import/document/commit', previewState.file, { 'x-source-checksum': previewState.sourceChecksum, 'x-import-checksum': previewState.checksum }); showMessage(`${committed.result.imported}개 자산을 등록했습니다.`); navigate('assets'); }
...
async function renderStocktakeDetail(id) {
  ...
  try { data=await request(`/api/enterprise/stocktakes/${id}`); }
    ...
    const snapshot=await offlineStore.loadSnapshot(id).catch(()=>null);
    if(!snapshot) throw error;
    data=snapshot.data; savedAt=snapshot.savedAt; offline=true;
  }
  offlineStore.assertActive();
  if(!offline) await offlineStore.saveSnapshot(id,data).catch(()=>null);
  const queued=await offlineStore.listOperations(id).catch(()=>[])||[];
  offlineStore.assertActive();
...
async function renderReports(filters = {}) {
  ...
  const exportLink = format => `/api/enterprise/reports/assets.${format}${query ? `?${query}` : ''}`;
  $('#view-root').innerHTML=`<div class="page-heading"><div><p class="eyebrow">MULTI-DIMENSION ASSET REPORT</p><h1>자산 운영 보고서</h1><p class="muted">부서·위치·유형·상태·취득기간을 같은 기준으로 집계합니다.</p></div><div class="export-actions" aria-label="보고서 내보내기"><a class="secondary button-link" href="${exportLink('xlsx')}">Excel</a><a class="secondary button-link" href="${exportLink('docx')}">Word</a><a class="secondary button-link" href="${exportLink('pptx')}">PPT</a><a class="secondary button-link" href="${exportLink('jpeg')}">JPEG</a><a class="primary button-link" href="${exportLink('png')}">PNG</a></div></div><section class="panel form-panel"><div><h2>보고 조건</h2><p class="muted">조건을 비우면 전체 조직을 조회합니다.</p></div><form id="report-filter" class="grid-form enterprise-form"><label>부서<select name="departmentId"><option value="">전체</option>${options(ref.departments,row=>row.name,filters.departmentId)}</select></label><label>위치<select name="locationId"><option value="">전체</option>${options(ref.locations,row=>row.name,filters.locationId)}</select></label><label>유형<select name="categoryId"><option value="">전체</option>${options(ref.categories,row=>row.name,filters.categoryId)}</select></label><label>상태<select name="status"><option value="">전체</option>${statuses.map(status=>`<option ${filters.status===status?'selected':''}>${status}</option>`).join('')}</select></label><label>취득 시작일<input type="date" name="from" value="${escapeHtml(filters.from||'')}"></label><label>취득 종료일<input type="date" name="to" value="${escapeHtml(filters.to||'')}"></label><button class="primary full">보고서 조회</button></form></section><section class="metric-strip"><article class="metric-feature"><span>필터 자산</span><strong>${s.assets}</strong></article><article><span>가용</span><strong>${s.available}</strong></article><article><span>사용 중</span><strong>${s.in_use}</strong></article><article class="metric-risk"><span>수리 / 분실 / 폐기대기</span><strong>${Number(s.repair)+Number(s.lost)+Number(s.dispose_pending)}</strong></article></section><section class="panel report-amount"><p class="eyebrow">FILTERED ACQUISITION VALUE</p><h2>${Number(s.total_cost||0).toLocaleString()}원</h2></section><section class="two-column">${dimension('부서별',data.breakdowns.departments)}${dimension('위치별',data.breakdowns.locations)}${dimension('유형별',data.breakdowns.categories)}${dimension('상태별',data.breakdowns.statuses)}</section>`;
