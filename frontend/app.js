...
async function renderAdmin() {
  ...
  const [data,referenceData]=await Promise.all([request('/api/enterprise/admin'),request('/api/enterprise/admin/references')]);
  ...
  document.querySelectorAll('.reference-update').forEach(button=>button.addEventListener('click',async()=>{const nameInput=document.querySelector(`.reference-name[data-kind="${button.dataset.kind}"][data-id="${button.dataset.id}"]`);const activeInput=document.querySelector(`.reference-active[data-kind="${button.dataset.kind}"][data-id="${button.dataset.id}"]`);try{await runLocked(button,async()=>{await reauthReference();await request(`/api/enterprise/admin/references/${button.dataset.kind}/${button.dataset.id}`,{method:'PATCH',body:{organizationId:state.user.organizationId,name:nameInput.value,isActive:activeInput.checked}});});showMessage('기준정보를 저장했습니다.');renderAdmin();}catch(error){showMessage(error.message,'error');}}));
}
