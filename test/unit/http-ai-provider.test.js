...
test('HTTP AI provider는 추천·OCR·health 계약과 인증 헤더를 지킨다', async () => {
  ...
  const provider = createHttpAiProvider({
    ...
    if (options.method === 'GET') return response({ status: url.endsWith('/ready') ? 'ready' : 'ok' });
    if (url.endsWith('/ocr')) return response({ fields: { assetTag: 'IT-001' }, confidence: { assetTag: 0.94 }, usage: { total_tokens: 11 } });
    return response({ provider: 'office-ai-bridge', modelVersion: 'pilot-v1', recommendations: [{ assetId: 7, actionType: 'TRANSFER', estimatedCost: 10, avoidedCost: 100, confidence: 0.91, evidence: ['idle'] }], usage: { total_tokens: 22 } });
  });
  ...
  const readiness = await provider.readinessCheck();
  assert.equal(recommendation.recommendations[0].assetId, 7);
  assert.equal(recommendation.provider, 'office-ai-bridge');
  assert.equal(recommendation.modelVersion, 'pilot-v1');
  ...
  assert.deepEqual(JSON.parse(calls[0].options.body), { organizationId: 3, query: {}, assets: [{ id: 7 }] });
