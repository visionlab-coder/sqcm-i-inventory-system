...
test('프런트엔드는 모바일 내비게이션·비용 화면 계약을 갖는다', () => {
  ...
  assert.match(app,/aria-labelledby', 'cost-position-title/); assert.match(app,/class="position-copy"/); assert.match(css,/\.product-position \{ display:grid; grid-template-columns:minmax\(0,1fr\) auto/); assert.match(css,/\.page-heading>button \{ flex:0 0 auto; white-space:nowrap; \}/);
});
