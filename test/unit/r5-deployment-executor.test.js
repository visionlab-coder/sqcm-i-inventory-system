...
function fakeDriver(fail) {
 const calls=[];
 const fields={freeze:{ingressBlocked:true,workersStopped:true,activeWritesZero:true},backup:{restoreVerified:true,runtimeFilesPreserved:true,backupDigest:digest,backupReference:'synthetic-memory'},migrate:{historyVerified:true,seedsDisabled:true},switchImages:{imageIdsMatched:true,threeServices:true,privateDatabasePorts:true},verify:{health:true,authenticatedSmoke:true,database:true,tls:true},release:{writesEnabled:true},rollback:{ingressBlocked:true,originalRuntimeRestored:true,candidateDataPreserved:true,backupDigest:digest}};
...
test('R5 verified sequence opens writes but never fabricates employee UAT',async()=>{
 ...
 const result=await executeR5Deployment({approval,candidate,driver,now,execute:true});assert.equal(result.status,'DEPLOYED_UAT_PENDING');assert.equal(result.employeeUat,'NOT_RUN');assert.deepEqual(calls,['freeze','backup','migrate','switchImages','verify','release']);
});
