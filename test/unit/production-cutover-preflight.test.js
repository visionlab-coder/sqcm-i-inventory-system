...
import { evaluateProductionCutoverPreflight } from '../../src/operations/production-cutover-preflight.js';
...
test('변경창 전 정상 내부 Production은 READY_WAIT_CHANGE_WINDOW다', () => {
...
test('변경창 안에서는 미게시 외부 항목을 실행 가능한 READY로 분리한다', () => {
...
test('모든 실제 외부 증거가 있으면 최종 서명 READY다', () => {
  const result = evaluateProductionCutoverPreflight(readyObservation({
...
test('로컬 불변식 손상은 변경창과 무관하게 fail closed 한다', () => {
  const result = evaluateProductionCutoverPreflight(readyObservation({
