...
test('변경창 안 공개 인증 idempotency는 exact Production HTTPS만 사용한다',async()=>{const {selectAuthenticatedIdempotencyTarget}=await modulePromise;const result=selectAuthenticatedIdempotencyTarget({...window,publicMode:true,now:new Date('2026-09-11T21:00:00+09:00')});assert.equal(result.target,'https://inventory.safe-link.co.kr');assert.equal(result.actualProductionGate,true);});
