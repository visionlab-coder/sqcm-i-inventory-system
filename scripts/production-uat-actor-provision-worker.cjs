...
(async()=>{
  ...
  try{
    ...
    for(const actor of input.actors){
      ...
      failureStep=`${role}_IDENTITY_LOOKUP`;
      ...
      failureStep=`${role}_SESSION_REVOKE`;
      await client.query("DELETE FROM user_sessions WHERE sess->>'userId'=$1",[String(userId)]);
      failureStep=`${role}_AUDIT`;
      await client.query(`INSERT INTO audit_logs(actor_user_id,action,entity_type,entity_id,metadata,request_id,organization_id)
        SELECT NULL,'PRODUCTION_UAT_ACTOR_PROVISIONED','USER',$1::varchar,$2::jsonb,$3::varchar,$4::bigint
        WHERE NOT EXISTS(SELECT 1 FROM audit_logs WHERE request_id=$3::varchar)`,[String(userId),JSON.stringify({role,scopeType,approvalId:input.approvalId}),`p6-uat-provision-${input.approvalId}-${role}`,organizationId]);
      roles.push(role);
    }
    failureStep='POSTCONDITION_COUNTS';
  ...
  }catch(error){await client.query('ROLLBACK');error.failureStep=failureStep;throw error;}finally{await client.end();}
})().catch((error)=>{console.error(JSON.stringify({status:'FAIL_UAT_ACTOR_WORKER',failureStep:/^[A-Z0-9_]+$/.test(error.failureStep||'')?error.failureStep:'UNKNOWN',databaseCode:/^[A-Z0-9]{5}$/.test(error.code||'')?error.code:null,secretValuesReadOrRecorded:false}));process.exit(1);});
