...
try {
  ...
  if (resolved.toLowerCase().startsWith(`${root.toLowerCase()}${path.sep}`)) throw new Error('ACTUAL_EVIDENCE_MUST_BE_EXTERNAL');
  ...
  const actualEvidence = JSON.parse(raw.toString('utf8'));
  const validation = validateActualOperationsHandoverEvidence(actualEvidence, { documents: loadActualOperationsHandoverBundle(actualEvidence, { baseDir: path.dirname(resolved) }) });
  const roadmap = JSON.parse(fs.readFileSync(files.roadmap, 'utf8'));
  const queue = JSON.parse(fs.readFileSync(files.queue, 'utf8'));
  const result = evaluateP7Completion({ roadmap, queue, actualEvidence, actualEvidenceSha256: createHash('sha256').update(raw).digest('hex'), validation, execute: process.argv.includes('--complete'), confirmation: process.env.P7_COMPLETION_CONFIRMATION });
