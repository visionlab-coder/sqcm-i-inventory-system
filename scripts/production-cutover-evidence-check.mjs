...
const expected = assembleProductionCutoverEvidence({ g3, g4, p5 });

assert.deepEqual(candidate, expected, 'Cutover evidence candidate drifted from verified source evidence.');
assert.equal(candidate.localGatePassCount, 3);
assert.equal(candidate.pendingGateCount, 9);
...
for (const id of ['provider_preflight', 'health_readiness', 'core_smoke', 'uat_signoff']) {
