...
import {
  ...
  OPERATIONS_SIGNOFF_EVIDENCE_CONFIRMATION,
...
const inputPath = process.env.P7_OPERATIONS_SIGNOFF_INPUT_FILE ? path.resolve(process.env.P7_OPERATIONS_SIGNOFF_INPUT_FILE) : null;
const outputPath = process.env.P7_OPERATIONS_SIGNOFF_EVIDENCE_OUTPUT_FILE ? path.resolve(process.env.P7_OPERATIONS_SIGNOFF_EVIDENCE_OUTPUT_FILE) : null;
const confirmed = process.env.P7_OPERATIONS_SIGNOFF_EVIDENCE_CONFIRMATION === OPERATIONS_SIGNOFF_EVIDENCE_CONFIRMATION;
...
const gate = evaluateOperationsSignoffEvidenceCompiler({
...
if (status === 'READY_OPERATIONS_SIGNOFF_EVIDENCE_COMPILATION') {
  try {
    ...
    status = 'BLOCKED_OPERATIONS_SIGNOFF_EVIDENCE_COMPILATION';
    failureCount = 1;
  }
}
...
console.log(JSON.stringify({
  ...
  status,
  requiredInputEnvironment: 'P7_OPERATIONS_SIGNOFF_INPUT_FILE',
  requiredOutputEnvironment: 'P7_OPERATIONS_SIGNOFF_EVIDENCE_OUTPUT_FILE',
  confirmationEnvironment: 'P7_OPERATIONS_SIGNOFF_EVIDENCE_CONFIRMATION',
  targetUrl: OPERATIONS_SIGNOFF_TARGET_URL,
  requiredDutyCount: OPERATIONS_SIGNOFF_DUTIES.length,
  ...
  externalSignatureCreatedOrChanged: false,
  externalMutationPerformed: false,
  productionGo: roadmap.invariants?.productionGo === true
}, null, 2));

if (status === 'BLOCKED_OPERATIONS_SIGNOFF_EVIDENCE_COMPILATION') process.exitCode = 1;
