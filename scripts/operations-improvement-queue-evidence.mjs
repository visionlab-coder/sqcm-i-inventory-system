...
import {
  IMPROVEMENT_QUEUE_EVIDENCE_CONFIRMATION,
...
const inputPath = process.env.P7_IMPROVEMENT_QUEUE_INPUT_FILE ? path.resolve(process.env.P7_IMPROVEMENT_QUEUE_INPUT_FILE) : null;
const outputPath = process.env.P7_IMPROVEMENT_QUEUE_EVIDENCE_OUTPUT_FILE ? path.resolve(process.env.P7_IMPROVEMENT_QUEUE_EVIDENCE_OUTPUT_FILE) : null;
const confirmed = process.env.P7_IMPROVEMENT_QUEUE_EVIDENCE_CONFIRMATION === IMPROVEMENT_QUEUE_EVIDENCE_CONFIRMATION;
...
const gate = evaluateOperationsImprovementQueueEvidenceCompiler({
...
if (status === 'READY_IMPROVEMENT_QUEUE_EVIDENCE_COMPILATION') {
  try {
    ...
    status = 'BLOCKED_IMPROVEMENT_QUEUE_EVIDENCE_COMPILATION';
    failureCount = 1;
  }
}
...
console.log(JSON.stringify({
  ...
  status,
  requiredInputEnvironment: 'P7_IMPROVEMENT_QUEUE_INPUT_FILE',
  requiredOutputEnvironment: 'P7_IMPROVEMENT_QUEUE_EVIDENCE_OUTPUT_FILE',
  confirmationEnvironment: 'P7_IMPROVEMENT_QUEUE_EVIDENCE_CONFIRMATION',
  requiredQueueRef: IMPROVEMENT_QUEUE_REF,
  ...
  externalIssueCreatedOrChanged: false,
  externalMutationPerformed: false,
  productionGo: roadmap.invariants?.productionGo === true
}, null, 2));

if (status === 'BLOCKED_IMPROVEMENT_QUEUE_EVIDENCE_COMPILATION') process.exitCode = 1;
