...
try {
  ...
  const migrations = await pool.query("SELECT count(*)::int count FROM supabase_migrations.schema_migrations WHERE name LIKE 'sqcmi_%'"); expect(migrations.rows[0].count === 27, 'Supabase migration count is not 27');
  ...
  record('P5-UAT-17', 'PASS', `health/readiness 200, migrations 27, audit 5xx marker count ${recent5xx.rows[0].count}`);
