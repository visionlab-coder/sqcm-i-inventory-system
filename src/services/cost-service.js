...
async function getCostCommandCenter(pool, user, organizationInput, scope = {}) {
  ...
  const [summary, monthly, idle, warranties, budgets] = await Promise.all([
    pool.query(`SELECT count(*)::int asset_count,
      ...
      COALESCE((SELECT sum(e.amount) FROM asset_cost_events e WHERE e.organization_id=$1 AND e.event_type='REPAIR'),0)::numeric repair_cost,
      COALESCE((SELECT sum(e.amount) FROM asset_cost_events e WHERE e.organization_id=$1 AND e.event_type='TRANSFER'),0)::numeric transfer_cost,
      COALESCE((SELECT sum(e.amount) FROM asset_cost_events e WHERE e.organization_id=$1 AND e.event_type='DISPOSAL'),0)::numeric disposal_cost,
    ...
    pool.query(`SELECT to_char(date_trunc('month',e.occurred_at),'YYYY-MM') AS period_month,COALESCE(sum(e.amount),0)::numeric amount,e.event_type
      FROM asset_cost_events e WHERE e.organization_id=$1 GROUP BY 1,e.event_type ORDER BY 1 DESC LIMIT 24`, [organizationId]),
  ...
  return { organizationId, summary: { ...row, tco: ['acquisition_cost','repair_cost','transfer_cost','disposal_cost'].reduce((sum,key)=>sum+Number(row[key]||0),0), book_value:Math.max(0,Number(row.acquisition_cost||0)-Number(row.depreciation||0)) }, monthly: monthly.rows, idleAssets: idle.rows, upcomingRenewals: warranties.rows, budgets: budgets.rows };
}
