...
function createEnterpriseRouter({ pool, apiAuth, requireRecentReauth }) {
  ...
  router.get('/admin', async(req,res)=>{ requirePermission(req.user,'admin.manage'); const [orgs,depts,locations,users,outbox]=await Promise.all([pool.query('SELECT * FROM organizations ORDER BY name'),pool.query('SELECT * FROM departments ORDER BY name'),pool.query('SELECT * FROM locations ORDER BY name'),pool.query('SELECT id,email,display_name,role,status,organization_id,department_id,mfa_enabled FROM users ORDER BY display_name'),pool.query('SELECT * FROM outbox_events ORDER BY created_at DESC LIMIT 50')]); res.json({organizations:orgs.rows,departments:depts.rows,locations:locations.rows,users:users.rows,outbox:outbox.rows}); });
  router.post('/admin/departments', async(req,res)=>{ requirePermission(req.user,'admin.manage'); const organizationId=orgId(req,req.body.organizationId); const result=await pool.query(`INSERT INTO departments(organization_id,parent_id,code,name,cost_center) VALUES($1,$2,$3,$4,$5) RETURNING *`,[organizationId,req.body.parentId||null,String(req.body.code||'').trim().toUpperCase(),String(req.body.name||'').trim(),String(req.body.costCenter||'').trim()||null]); await audit(pool,req,'DEPARTMENT_CREATED','DEPARTMENT',result.rows[0].id); res.status(201).json({department:result.rows[0]}); });
