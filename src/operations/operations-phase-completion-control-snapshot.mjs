...
export const OPERATIONS_PHASE_COMPLETION_CONTROL_MAX_BYTES = 1024 * 1024;

const CONTROL_FILES = {
  roadmap: ['agent docs', 'harness', 'MASTER_ROADMAP.json'],
  queue: ['agent docs', 'harness', 'P6_P7_ACCELERATION_QUEUE.json']
};

function controlError(code) {
  const error = new Error(code);
  error.name = 'OperationsPhaseCompletionControlError';
  return error;
}
...
function inspect(root, files, io) {
  let rootStat; let rootReal; const entries = {};
  try {
    rootStat = io.lstatSync(root);
    rootReal = path.resolve(io.realpathSync(root));
    for (const [name, file] of Object.entries(files)) {
      entries[name] = { stat: io.lstatSync(file), real: path.resolve(io.realpathSync(file)) };
    }
  } catch {
    throw controlError('OPERATIONS_PHASE_COMPLETION_CONTROL_NOT_FOUND');
  }
  return { rootStat, rootReal, entries };
}

function decodeObject(raw) {
  let source;
  try { source = new TextDecoder('utf-8', { fatal: true }).decode(raw); }
  catch { throw controlError('OPERATIONS_PHASE_COMPLETION_CONTROL_UTF8_INVALID'); }
  let value;
  try { value = JSON.parse(source); }
  catch { throw controlError('OPERATIONS_PHASE_COMPLETION_CONTROL_JSON_INVALID'); }
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw controlError('OPERATIONS_PHASE_COMPLETION_CONTROL_JSON_INVALID');
  }
  return value;
}

export function readOperationsPhaseCompletionControlSnapshot(projectRoot, {
  io = fs,
  maxBytes = OPERATIONS_PHASE_COMPLETION_CONTROL_MAX_BYTES
} = {}) {
  if (typeof projectRoot !== 'string' || !path.isAbsolute(projectRoot)
    || !Number.isInteger(maxBytes) || maxBytes < 1
    || maxBytes > OPERATIONS_PHASE_COMPLETION_CONTROL_MAX_BYTES) {
    throw controlError('OPERATIONS_PHASE_COMPLETION_CONTROL_REFERENCE_INVALID');
  }

  const root = path.resolve(projectRoot);
  const files = Object.fromEntries(
    Object.entries(CONTROL_FILES).map(([name, segments]) => [name, path.join(root, ...segments)])
  );
  const before = inspect(root, files, io);
  if (!physicalDirectory(before.rootStat) || !samePath(before.rootReal, root)) {
    throw controlError('OPERATIONS_PHASE_COMPLETION_CONTROL_ROOT_NOT_PHYSICAL');
  }
  for (const [name, file] of Object.entries(files)) {
    const entry = before.entries[name];
    if (!physicalFile(entry.stat) || !samePath(entry.real, file)) {
      throw controlError('OPERATIONS_PHASE_COMPLETION_CONTROL_FILE_NOT_PHYSICAL');
    }
    if (entry.stat.size < 1 || entry.stat.size > maxBytes) {
      throw controlError('OPERATIONS_PHASE_COMPLETION_CONTROL_SIZE_INVALID');
    }
  }
  ...
  try {
    ...
    throw controlError('OPERATIONS_PHASE_COMPLETION_CONTROL_READ_FAILED');
  }
  ...
  if (!physicalDirectory(after.rootStat)
    ...
    throw controlError('OPERATIONS_PHASE_COMPLETION_CONTROL_UNSTABLE');
  }
  for (const [name, file] of Object.entries(files)) {
