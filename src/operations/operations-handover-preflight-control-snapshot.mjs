...
export const OPERATIONS_HANDOVER_PREFLIGHT_CONTROL_MAX_BYTES = 1024 * 1024;

const CONTROL_FILES = {
  candidate: ['agent docs', 'harness', 'P7_OPERATIONS_HANDOVER_PREFLIGHT_CANDIDATE.json'],
  roadmap: ['agent docs', 'harness', 'MASTER_ROADMAP.json']
};

function controlError(code) {
  const error = new Error(code);
  error.name = 'OperationsHandoverPreflightControlError';
  return error;
}
...
function inspect(root, files, io) {
  ...
  try {
    ...
    throw controlError('OPERATIONS_HANDOVER_PREFLIGHT_CONTROL_NOT_FOUND');
  }
...
function decodeObject(raw) {
  ...
  catch { throw controlError('OPERATIONS_HANDOVER_PREFLIGHT_CONTROL_UTF8_INVALID'); }
  let value;
  try { value = JSON.parse(source); }
  catch { throw controlError('OPERATIONS_HANDOVER_PREFLIGHT_CONTROL_JSON_INVALID'); }
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw controlError('OPERATIONS_HANDOVER_PREFLIGHT_CONTROL_JSON_INVALID');
  }
  return value;
}

export function readOperationsHandoverPreflightControlSnapshot(projectRoot, {
  io = fs,
  maxBytes = OPERATIONS_HANDOVER_PREFLIGHT_CONTROL_MAX_BYTES
} = {}) {
  if (typeof projectRoot !== 'string' || !path.isAbsolute(projectRoot)
    || !Number.isInteger(maxBytes) || maxBytes < 1
    || maxBytes > OPERATIONS_HANDOVER_PREFLIGHT_CONTROL_MAX_BYTES) {
    throw controlError('OPERATIONS_HANDOVER_PREFLIGHT_CONTROL_REFERENCE_INVALID');
  }
  ...
  if (!physicalDirectory(before.rootStat) || !samePath(before.rootReal, root)) {
    throw controlError('OPERATIONS_HANDOVER_PREFLIGHT_CONTROL_ROOT_NOT_PHYSICAL');
  }
  for (const [name, file] of Object.entries(files)) {
    const entry = before.entries[name];
    if (!physicalFile(entry.stat) || !samePath(entry.real, file)) {
      throw controlError('OPERATIONS_HANDOVER_PREFLIGHT_CONTROL_FILE_NOT_PHYSICAL');
    }
    if (entry.stat.size < 1 || entry.stat.size > maxBytes) {
      throw controlError('OPERATIONS_HANDOVER_PREFLIGHT_CONTROL_SIZE_INVALID');
    }
  }
  ...
  try {
    ...
    throw controlError('OPERATIONS_HANDOVER_PREFLIGHT_CONTROL_READ_FAILED');
  }
  ...
  if (!physicalDirectory(after.rootStat)
    ...
    throw controlError('OPERATIONS_HANDOVER_PREFLIGHT_CONTROL_UNSTABLE');
  }
  for (const [name, file] of Object.entries(files)) {
